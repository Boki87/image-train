...
async function doActions(actions: [{type:string, params:SharpParam}], images: [string]):Promise<void> {
  try {
    //loop trough all the images
    for (let i = 0; i < images.length; i++) {
      //Duplicate image to work with
      let image:string = images[i];
      let imageExtension: any = path.extname(image);
      let imageName = path.basename(image).split(imageExtension)[0];
      // console.log(imageName, imageExtension);

      let duplicateImagePath = path.join(
        __dirname,
        "temp",
        imageName + i + "0" + imageExtension
      );
      ...
      for (let a = 0; a < actions.length; a++) {
        let tempIncrementImage = path.join(
          __dirname,
          "temp",
          imageName + i.toString() + a.toString() + imageExtension
        );
        ...
        switch (action.type) {
          case "resize":
            await sharp(tempIncrementImage)
              .resize(action.params)
              .toFile(tempIncrementedImage);
          ...
          case "toFormat":
            ...
            imageExtension = action.params.format;
